...
class CatList {
	...
	private function check_pagination($args) {
		if ($this->utils->lcp_not_empty('pagination')) {
			if (array_key_exists('QUERY_STRING', $_SERVER) && null !== $_SERVER['QUERY_STRING']) {
				...
				if ($query !== '' && preg_match('/lcp_page' . preg_quote($this->instance) . '=([0-9]+)/i', $query, $match)) {
					$this->page = $match[1];
	...
	private function lcp_categories() {
		if (is_array($this->lcp_category_id)) {
			return array(
				'category__and' => $this->lcp_category_id
			);
		} else {
			if ($this->utils->lcp_not_empty('child_categories') && (($this->params['child_categories'] === 'no') || ($this->params['child_categories'] === 'false'))) {
				return array(
					'category__in' => $this->lcp_category_id
				);
			}
			return array(
				'cat' => $this->lcp_category_id
			);
		}
	}
	...
	public function get_category_link() {
		if (($this->utils->lcp_not_empty('catlink') && $this->params['catlink'] == 'yes' || $this->utils->lcp_not_empty('catname') && $this->params['catname'] == 'yes') && $this->lcp_category_id != 0) {
			...
			foreach ($ids as $lcp_id) {
				...
				if ($this->params['catlink'] == 'yes') {
					$cat_string = '<a href="' . $cat_link . '" title="' . $cat_title . '">' . $cat_string . $this->get_category_count() . '</a>';
				}
	...
	public function get_author_to_show($single) {
		if ($this->params['author'] == 'yes') {
			...
			if ($this->utils->lcp_not_empty('author_posts_link') && $this->params['author_posts_link'] == 'yes') {
	...
	public function get_content($single) {
		if (isset($this->params['content']) && ($this->params['content'] == 'yes' || $this->params['content'] == 'full') && $single->post_content) {
			$lcp_content = $single->post_content;
			$lcp_content = apply_filters('the_content', $lcp_content);
			$lcp_content = str_replace(']]>', ']]&gt', $lcp_content);
			if ($this->params['content'] == 'yes' && preg_match('/[\S\s]+(<!--more(.*?)?-->)[\S\s]+/', $lcp_content, $matches)) {
				...
				$lcp_content      = $lcp_post_content[0] . ($lcp_more ? ' <a href="' . get_permalink($single->ID) . '" title="' . "$lcp_more" . '">' . $lcp_more . '</a>' : '');
			}
	...
	public function lcp_get_excerpt($single) {
		if (!empty($this->params['excerpt']) && ($this->params['excerpt'] == 'yes' || $this->params['excerpt'] == 'full')) {
			
			if ($single->post_excerpt == "" || (!empty($this->params['excerpt_overwrite']) && $this->params['excerpt_overwrite'] == 'yes')) {
				// No explicit excerpt or excerpt_overwrite=yes, so generate from content:
				$lcp_content = $single->post_content;
				// <!--more--> tag?
				if ($this->params['excerpt'] == 'full' && preg_match('/[\S\s]+(<!--more(.*?)?-->)[\S\s]+/', $lcp_content, $matches)) {
	...
	private function lcp_trim_excerpt($text = '') {
		...
		if ($this->utils->lcp_not_empty('excerpt_strip') && $this->params['excerpt_strip'] == 'yes') {
	...
	public function get_thumbnail($single, $lcp_thumb_class = null) {
		...
		return LcpThumbnail::get_instance()->get_thumbnail($single, $this->params['thumbnail'], $this->params['thumbnail_size'], $force_thumbnail, $lcp_thumb_class);
	}
